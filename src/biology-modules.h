...
struct ApicalElongationBM : public BaseBiologyModule {
  ...
  void Run(T* dendrite) {
    ...
    ofstream Neural_Network_Morphology_from_BM_12_06_2019;
    Neural_Network_Morphology_from_BM_12_06_2019.open("Neuron_morphology_from_BM",ios::out | ios::app);
    ...
    if (dendrite->GetDiameter() > 0.5) {
      ...
      dendrite->SetDiameter(dendrite->GetDiameter() - 0.001);
                    ...
                    auto position = dendrite->GetPosition();
                //cout<<"BM_DIAMETER:     "<< dendrite->GetDiameter()<< "     BM_SO_PTR:    "<<dendrite->GetSoPtr()<<"    BM_POSITION:  "<<position[0]<<"  "<<position[1]<<"  "<<position[2] <<endl;
        Neural_Network_Morphology_from_BM_12_06_2019<<"BM_DIAMETER:     "<< dendrite->GetDiameter()<< "     BM_SO_PTR:    "<<dendrite->GetSoPtr()<<"    BM_POSITION:  "<<position[0]<<"  "<<position[1]<<"  "<<position[2]<<endl;
